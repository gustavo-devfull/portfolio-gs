  ...
  try {
    ...
    for (const project of projects) {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          fields: convertToFirestoreObj(project),
        }),
      })

      if (!response.ok) {
        const error = await response.json()
        throw new Error(`Erro ao adicionar ${project.title.pt}: ${JSON.stringify(error)}`)
      }

      const data = await response.json()
      console.log(`✅ Projeto criado: ${project.title.pt} (${data.name.split('/').pop()})`)
    }
...
function convertToFirestoreValue(val) {
  ...
  if (typeof val === 'object') {
    return { mapValue: { fields: convertToFirestoreObj(val) } }
  }
  return { nullValue: null }
}

function convertToFirestoreObj(obj) {
  const fields = {}
